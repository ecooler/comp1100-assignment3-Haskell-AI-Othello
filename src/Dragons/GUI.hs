    ...
    DrawPiece p (x, y) -> do
      let startColour = case p of
          ...
          stopColour = case p of
                    Dark -> "black"
                    Light -> "gray"
          screenX = fromIntegral x * cellSize + (cellSize/2)
          screenY = fromIntegral y * cellSize + (cellSize/2)
      moveToBoard
      arc (screenX, screenY, r, 0, 2*pi, False)
      grad <- createRadialGradient (screenX, screenY, 0, screenX, screenY,r)
      ...
      thickBlackStroke
    BadMove (x, y) -> do
      let screenX = fromIntegral x * cellSize + (cellSize/2)
          screenY = fromIntegral y * cellSize + (cellSize/2)
      moveToBoard
      grad <- createRadialGradient (screenX, screenY, 0, screenX, screenY, r)
      arc (screenX, screenY, r, 0, 2*pi, False)
...
-- | Create a pattern from a simple linear gradient from the size of
-- the "texture" and the gradient stops.
